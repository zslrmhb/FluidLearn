...
def generate_symbolic_binding_dataset(
    ...
    for difficulty, count in difficulty_counts.items():
        if difficulty not in SYMBOLIC_BINDING_DIFFICULTY:
            raise ValueError(f"Unknown difficulty level: {difficulty}")

        profile = SYMBOLIC_BINDING_DIFFICULTY[difficulty]

        for _ in range(count):
            instance_seed = rng.randint(0, 10**9)
            episode = generate_symbolic_binding_episode(
                profile=profile,
                seed=instance_seed,
