...
def generate_feedback_exploration_dataset(
    ...
    for difficulty, count in difficulty_counts.items():
        if difficulty not in FEEDBACK_EXPLORATION_DIFFICULTY:
            raise ValueError(f"Unknown difficulty level: {difficulty}")

        profile = FEEDBACK_EXPLORATION_DIFFICULTY[difficulty]

        for _ in range(count):
            instance_seed = rng.randint(0, 10**9)
            episode = generate_feedback_exploration_episode(
                profile=profile,
                seed=instance_seed,
