...
def generate_compositional_induction_dataset(
    ...
    for difficulty, count in difficulty_counts.items():
        if difficulty not in COMPOSITIONAL_INDUCTION_DIFFICULTY:
            raise ValueError(f"Unknown difficulty level: {difficulty}")

        profile = COMPOSITIONAL_INDUCTION_DIFFICULTY[difficulty]

        for _ in range(count):
            instance_seed = rng.randint(0, 10**9)
            episode = generate_compositional_induction_episode(
                profile=profile,
                seed=instance_seed,
